...
function M.setup(config)
  if check_config(config) then
    ...
    vim.notify(
      "Kanagawa: Errors found while loading user config. Using default config.",
...
function M.load(theme)
  ...
  if M.config.compile then
    ...
    local colors =
      require("kanagawa.colors").setup { theme = theme, colors = M.config.colors }
    local highlights = require("kanagawa.highlights").setup(colors, M.config)
    require("kanagawa.highlights").highlight(
      highlights,
...
function M.compile()
  for theme, _ in pairs(require "kanagawa.themes") do
    local colors =
      require("kanagawa.colors").setup { theme = theme, colors = M.config.colors }
    local highlights = require("kanagawa.highlights").setup(colors, M.config)
    require("kanagawa.utils").compile(
      theme,
      highlights,
      M.config.terminalColors and colors.theme.term or {}
    )
